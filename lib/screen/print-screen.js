...
export const resetCursor = () => {
    const ready = Screen.screenStream.cursorTo(0, 0);
    if (ready)
        return Promise.resolve();
    else
        return new Promise((res, rej) => void Screen.screenStream.once('drain', res));
}


/**
 * @typedef PrintOptions
 * @property {?Boolean} resetCursor
 */
 ...
 */
export const printScreen = async (buffer, options) => {
    if (!Screen.hasScreen)
        throw new Error("Screen not available");

    printBuff.reset();

    let [dx, dy] = buffer.bufferDim;
    dx = Math.min(dx, Screen.realScreenSize[0]), dy = Math.min(dy, Screen.realScreenSize[1]);
    ...
    printBuff.pushColor(CColor.white); // TODO use default color
    ...
    await new Promise((res, rej) => {
        Screen.screenStream.write(pbSlice, 'utf-8', res);
    });
