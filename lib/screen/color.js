 ...
 * Set the color of the following characters in the stream
 * @param {TextStreamWriter} stream The target stream
 * @param {ArrayLike<Number>} fgColor
 * @param {ArrayLike<Number>} bgColor
 */
const setColorAdvanced = (stream, fgColor, bgColor) => {
    if (!isSupported)
        return;

    if (fgColor != null) {
        const ccode = get256Color(fgColor[0], fgColor[1], fgColor[2]);

        write256Color(stream, true, ccode);
    }
    
    if (bgColor != null) {
        const ccode = get256Color(bgColor[0], bgColor[1], bgColor[2]);
