 ...
 * @returns {Number}
 */
export const getBufferIndex = (coord) => {
...
export const screenSize = () => bufferDim;
...
export class ScreenWriter
{
    ...
    /**
     * @typedef SWOptions
     * @type {?Boolean} zTestEnabled
     * @type {?Boolean} zWriteEnabled
    ...
    set symbol(value) { this.symbol_ = getAsciiCode(value); }
     ...
     * @param {ScreenCoord} coord The coordinate to write
     * @returns {Boolean}
     */
    write(coord) {
        const i = coord[0] + coord[1] * bufferDim[0];
     ...
     * @param {ScreenCoord} first The first coordinate to write
     * @param {ScreenCoord} second The second coordinate to write
     */
