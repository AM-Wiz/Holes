...
const printBuff = new class PrintBuffer
{
    ...
    pushChunk(chunk, start, count) {
        start ??= 0;
        count ??= chunk.length ?? chunk.byteLength;
        ...
        if (typeof chunk === 'string') {
            ...
            const { read } = this.encoder.encodeInto(chunk, destAr);

            assert.ok(read === count);
        } else {
            const cBuff = chunk instanceof ArrayBuffer ? chunk : chunk.buffer;

            assert.ok(cBuff instanceof ArrayBuffer);

            this.buffer.set(new Uint8Array(cBuff, start, count), this.cursor);
        }
...
const printScreen = () => {
    ...
    screenStream.write(pbSlice, 'ascii');
