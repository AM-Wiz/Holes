...
export class LoopEvent extends Event
{
    ...
    #onPollSchedule_(ts) {
        ...
        if (this.nextPoll_ > ts) {
            addPollDSet_(this.nextPoll_, this);
 ...
 * @param {ExAggr} exs 
 */
const pollDSet_ = (ts, exs) => {
    const lastPoll = lepeDirtyQueue.length;
    for (let i = 0; i < lastPoll; i++) {
        const e = lepeDirtyQueue[i];
    ...
    for (let i = lastPoll; i < lepeDirtyQueue.length; i++) {
        const e = lepeDirtyQueue[i];
        lepeQueue.push(e);
    }

    lepeDirtyQueue.splice(0, lepeDirtyQueue.length);
}
