...
export class LoopEvent extends Event
{
    ...
    set recurring(value) {
        ...
        if (this.recurring_ >= 0) {
            ...
            if (this.nextPoll_ != null)
                this.#setNextPoll(Math.min(this.nextPoll_, next));
            else
                this.#setNextPoll(next);
        }
    ...
    delayPoll(time) {
        this.#setNextPoll((this.nextPoll_ ?? this.curTs) + time);
     ...
     */
    requestPollIn(time) {
        const req = this.curTs + time;
        this.#setNextPoll(this.nextPoll_ == null ? req : Math.min(this.nextPoll_, req));
    ...
    requestPollBy(ts) {
        this.#setNextPoll(Math.min(this.nextPoll_ ?? this.curTs, ts));
...
const schedulingQuanta = 0.001;

const roundTs = (ts) => Math.trunc(ts / schedulingQuanta) * schedulingQuanta;

export const getTs = () => roundTs(process.uptime());
